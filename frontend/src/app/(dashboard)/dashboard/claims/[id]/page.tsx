  ...
  return(
    <div className="bg-white w-full border border-slate-200 rounded-xl p-6 transition-all duration-500">
       <div>
              ...
              <p className="text-slate-500 text-sm mt-1">View complete claim information.</p>
          </div>
            ...
            <div className="bg-white col-span-2 w-full border border-slate-200 rounded-xl p-6 transition-all duration-500">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-5">
                ...
                <div className="flex col-span-1 justify-between">
                  ...
                  <p className="font-semibold">{new Date(selectedClaim?.claimDate || "").toLocaleDateString()}</p>
                </div>
                <div className="flex col-span-1 justify-between">
                  <p className="block text-slate-500 font-semibold">Policy Number</p>
                  <p className="font-semibold">{selectedClaim?.policy?.policyNumber}</p>
                </div>
                <div className="flex col-span-1 justify-between">
                  <p className="block text-slate-500 font-semibold">Customer Name</p>
                ...
                <div className="flex col-span-1 justify-between">
                  <p className="block text-slate-500 font-semibold">Claimed Amount</p>
                  <p className="font-semibold"> ₹ {selectedClaim?.claimAmount.toLocaleString("en-IN")}</p>
                </div>
                <div className="flex col-span-1 justify-between">
                  <p className="block text-slate-500 font-semibold">Claimant Name</p>
             ...
             <div className="bg-white col-span-2 lg:col-span-1 w-full border border-slate-200 rounded-xl p-6 transition-all duration-500">
              ...
              <div className="flex justify-between mt-5">
                  ...
                  <p className="font-semibold">{selectedClaim?.policy?.product?.productName}</p>
              </div>
              <div className="flex justify-between mt-5">
                  ...
                  className={`px-2 py-1 rounded-full text-xs font-medium
                    ${
                      selectedClaim?.policy?.status?.statusName === "Active"
                        ...
                        ? "bg-red-100 text-red-700"
                        : selectedClaim?.policy?.status?.statusName === "Pending"
                        ? "bg-blue-100 text-blue-700"
                        : "bg-yellow-100 text-yellow-700"
                    }`}
                >
              ...
              <div className="flex justify-between mt-5">
                  ...
                  <p className="font-semibold">₹ {selectedClaim?.policy?.premium?.sumAssured.toLocaleString("en-IN")}</p>
              </div>
              <div className="flex justify-between mt-5">
                  <p className="block text-slate-500 font-semibold">Policy Start Date</p>
                  <p className="font-semibold">{new Date(selectedClaim?.policy?.commencementDate || "").toLocaleDateString()}</p>
              </div>
              <div className="flex justify-between mt-5">
                  <p className="block text-slate-500 font-semibold">Policy End Date</p>
                  <p className="font-semibold">{new Date(selectedClaim?.policy?.maturityDate || "").toLocaleDateString()}</p>
              </div>
              <div className="flex justify-between mt-5">
                  <p className="block text-slate-500 font-semibold">Premium Paying Term</p>
                  <p className="font-semibold">{selectedClaim?.policy?.premiumPayingTerm || "-"}</p>
              </div>
            </div>
          </div>
    </div>
