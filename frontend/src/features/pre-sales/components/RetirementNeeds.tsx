...
export default function RetirementNeedsCalculator() {
  ...
  // ── PDF download — same row-safe pagination pattern as the other calculators ──
        ...
        <div key={cat.key} className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_8px_24px_rgba(15,23,42,0.05)]">
          ...
          <div className="grid grid-cols-1 gap-4 p-5 sm:grid-cols-3">
            ...
            <div>
              ...
              <input readOnly value={`₹ ${fmt(annualCostAtRetirement(cat))}`} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm font-semibold text-slate-700" />
            </div>
          </div>
        </div>
      ))}
      ...
      <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_8px_24px_rgba(15,23,42,0.05)]">
        ...
        <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 font-serif text-2xl font-semibold text-slate-900">₹ {fmt(totalAnnualCost)}</p>
          </div>
          <button
