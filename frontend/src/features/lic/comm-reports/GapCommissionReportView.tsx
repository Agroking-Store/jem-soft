...
import {
  GapCommissionFormData,
  GapCommissionItem,
  generateGapCommissionItems,
  calculateGapCommissionTotals,
} from "./gapCommissionData";

interface GapCommissionReportViewProps {
  formData: GapCommissionFormData;
  policies?: any[];
  onBackToForm: () => void;
}

const ITEMS_PER_PAGE = 45; // Matching ~45-50 rows per page of authentic LIC statement

export default function GapCommissionReportView({
  formData,
  policies = [],
  onBackToForm,
}: GapCommissionReportViewProps) {
  ...
  const allItems: GapCommissionItem[] = useMemo(() => {
    return generateGapCommissionItems(policies, selectedAgencyNames, formData);
  }, [policies, selectedAgencyNames, formData]);

  // Grand totals of all matching records
  const totals = useMemo(() => {
    return calculateGapCommissionTotals(allItems);
  }, [allItems]);
  ...
  const handleDownloadPDF = async () => {
    ...
    setIsExporting(true);
    const toastId = toast.loading("Generating Gap Commission Statement PDF...");
    ...
    try {
      ...
      pdf.save(`Gap_Commission_Statement_${formData.reportDate.replace(/\//g, "-")}.pdf`);
      toast.success("Gap Commission Statement PDF downloaded successfully!", { id: toastId });
    } catch (err: unknown) {
      console.error(err);
      elem.style.width = originalWidth;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm print:hidden">
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-emerald-50 border border-emerald-200 text-xs font-semibold text-emerald-800">
            <span>Bonus Recble: <strong>₹{totals.totalBonusRecble.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</strong></span>
          </div>
        </div>
      ...
      >
        ...
        {currentPage === 1 ? (
          <div className="space-y-4 mb-4">
            ...
            <div className="border border-slate-900 py-2 px-3 text-center text-xs font-bold tracking-tight">
              <span className="text-base font-extrabold uppercase">
                Gap Commission Statement
              </span>
            </div>

            {/* Statement Date, Due Date Range, and Page indicator */}
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between text-[11px] font-semibold text-slate-800 pt-1 gap-1">
              <div>
                <span>Date : {formData.reportDate}</span>
                <span className="block text-slate-700">Due Date between {formData.dateFrom} and {formData.dateTo}</span>
              </div>
              <span className="self-end sm:self-auto font-bold">Page {currentPage} of {totalPages}</span>
            </div>
          </div>
        ) : (
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between text-[11px] font-bold border-b border-slate-300 pb-1.5 text-slate-800">
              <span>
                Page {currentPage} of {totalPages} Due Date between {formData.dateFrom} and {formData.dateTo} as on {formData.reportDate}
              </span>
              <span>Gap Commission</span>
            </div>
          </div>
        )}
          ...
          <table className="w-full border-collapse text-[10.5px]">
            <thead>
              <tr className="border-t border-b border-slate-800 text-[10px] font-bold text-slate-900 leading-tight">
                ...
                <th className="py-2 px-1 text-center whitespace-nowrap">Com<br/>Date</th>
                <th className="py-2 px-1 text-center whitespace-nowrap">Pl/Tm/Pt</th>
                <th className="py-2 px-1 text-center whitespace-nowrap">Mode</th>
                <th className="py-2 px-1 text-center whitespace-nowrap">Due Date</th>
                <th className="py-2 px-1 text-right whitespace-nowrap">Comsn.<br/>Recble.</th>
                <th className="py-2 px-1 text-right whitespace-nowrap">Bonus<br/>Recble.</th>
                <th className="py-2 px-1 text-center whitespace-nowrap">Comn<br/>Code</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 font-mono text-[10px]">
              {paginatedItems.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/70 transition-colors">
                  ...
                  <td className="py-1 px-1 text-left font-sans text-slate-900 truncate max-w-[190px]" title={item.holderName}>
                    {item.holderName}
                  </td>
                  ...
                  <td className="py-1 px-1 text-center text-slate-700 font-sans whitespace-nowrap">
                    {item.mode}
                  </td>
                  <td className="py-1 px-1 text-center text-slate-700 whitespace-nowrap">
                    {item.dueDate}
                  ...
                  <td className="py-1 px-1 text-right text-slate-900">
                    {item.bonusRecble.toFixed(2)}
                  </td>
                  <td className="py-1 px-1 text-center font-sans text-slate-800">
                    {item.comnCode}
                  </td>
                </tr>
              ))}
              ...
              {isLastPage && (
                <tr className="border-t-2 border-b-2 border-slate-900 font-bold text-[10.5px] bg-slate-50/50">
                  ...
                  <td className="py-2 px-1 text-right text-slate-900 font-extrabold">
                    {totals.totalBonusRecble.toFixed(2)}
                  </td>
                  <td></td>
                </tr>
              )}
