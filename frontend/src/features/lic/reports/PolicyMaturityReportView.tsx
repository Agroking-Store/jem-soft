...
export default function PolicyMaturityReportView({
  ...
  const columns: ColumnDef[] = useMemo(() => {
    ...
    if (formData.reportOptions.dob) {
      cols.push({ key: "dob", label: "D.O.B", render: (p) => p.dob });
    }
    if (formData.reportOptions.statementWithPan) {
      cols.push({ key: "pan", label: "PAN", render: (p) => p.pan || "-" });
    }
    cols.push(
  ...
  const groupData = useMemo(() => {
    ...
    const validDbPolicies = rawPolicies.filter((p) => {
      ...
      if (fromDate && toDate && maturityRaw) {
        const md = new Date(maturityRaw);
        if (!isNaN(md.getTime()) && (md < fromDate || md > toDate)) return false;
      }
      return true;
    });
    ...
    const buildRow = (p: any) => {
      ...
      const comDate = p.commencementDate ? new Date(p.commencementDate) : new Date("2015-01-20");
      const maturityDate = p.maturityDate ? new Date(p.maturityDate) : new Date(formData.fromMaturityDate || "2026-08-01");
      const completedYears = Math.max(
        1,
        Math.floor((maturityDate.getTime() - comDate.getTime()) / (365.25 * 24 * 60 * 60 * 1000))
      );
    ...
    if (validDbPolicies.length > 0) {
      ...
      validDbPolicies.forEach((p) => {
        const gCode = p.customer?.groupCode || `M${(p.clientId || "01").toString().padStart(3, "0")}`;
        ...
        if (!groupMap[gCode]) groupMap[gCode] = { groupCode: gCode, groupHeadName: gHeadName, membersMap: {}, totalPolicies: 0, totalNetPayable: 0 };
        const grp = groupMap[gCode];
        if (!grp.membersMap[memberName]) grp.membersMap[memberName] = { name: memberName, policies: [], totalNetPayable: 0 };
    ...
    return [
      {
        ...
        members: [{ name: `Mr ${selectedGroupHead.replace(/^Mrs?\s*/i, "")}`, policies: [demoRow], totalNetPayable: demoRow.netMaturityPayable }],
        totalPolicies: 1,
        totalNetPayable: demoRow.netMaturityPayable,
      },
    ];
