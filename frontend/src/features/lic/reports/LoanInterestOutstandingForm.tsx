...
interface LoanInterestOutstandingFormProps {
  onBack: () => void;
  onGenerateReport: (data: LoanInterestOutstandingFormData) => void;
  initialData?: LoanInterestOutstandingFormData | null;
  agencies: any[];
  policyStatuses: any[];
  customers: any[];
  policies: any[];
...
const defaultFormData = (): LoanInterestOutstandingFormData => ({
  ...
  selectedGroups: [],
  statementOptions: { statementWithAddress: false, dob: false, registerWithTelNos: true },
  intimationOptions: {
    mailingLabels: false,
    despatchList: false,
    dob: false,
      ...
      <div className="space-y-6">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {formData.reportType === "Statement" ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-y-3.5 gap-x-6 pt-2">
              {[
                { key: "statementWithAddress", label: "Statement with Address" },
                { key: "dob", label: "DOB" },
                { key: "registerWithTelNos", label: "Register with Tel Nos." },
              ].map((opt) => (
                <label
                  key={opt.key}
                  className="flex items-center gap-2.5 text-xs font-bold text-slate-800 hover:text-[#B8873A] cursor-pointer transition"
