...
import { runSchedulerScan, getUpcomingCelebrations } from "../services/schedulerService.js";
import { seedDefaultTemplates } from "../services/templateService.js";

export const fetchUpcomingCelebrations = async (req: Request, res: Response): Promise<void> => {
  try {
    const days = req.query.days ? parseInt(req.query.days as string, 10) : 30;
    const celebrations = await getUpcomingCelebrations(days);
    res.status(200).json({ success: true, data: celebrations });
  } catch (error: any) {
    console.error("Fetch Upcoming Celebrations Error:", error);
    res.status(500).json({ success: false, message: error.message || "Failed to fetch celebrations." });
  }
};
