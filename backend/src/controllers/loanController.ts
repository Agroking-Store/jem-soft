...
export const addLoan = async (req: Request, res: Response) => {
    try {
        const { policyId, loanAmount, loanDate, loanStatusId } = req.body;

        if (!policyId || !loanAmount || !loanDate || !loanStatusId) {
            res.status(400).json({
                success: false,
                message: 'policyId, loanAmount, loanDate and loanStatusId are required',
            });
            return;
        }
...
export const updateLoan = async (req: Request, res: Response) => {
    try {
        const updatedLoan = await loanService.updateLoanById(req.params.id, req.body);
