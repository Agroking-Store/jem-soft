...
export const seedRiderPremium736 = async (prisma: PrismaClient) => {
  ...
  try {
    ...
    if (!product) {
      console.log(`❌ Product with planNumber 736 not found in ProductMaster`);
      return;
    }
      ...
      for (const column of termColumns) {
        ...
        if (existing) {
          ...
          if (existingRate === rate) {
            ...
            console.log(
              `♻ Updated 736 Rate: Age ${entryAge}, Term ${riderTerm} | Old Rate: ${existingRate} | New Rate: ${rate}`,
            );
          }
