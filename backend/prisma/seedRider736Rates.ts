...
export const seedRiderPremium736 = async (prisma: PrismaClient) => {
  const sqlite = new Database("./prisma/Creations.db", {
    readonly: true,
  });

  const tableName = "TermRider_736";
  ...
  try {
    ...
    for (const row of premiumRows) {
      ...
      for (const column of termColumns) {
        ...
        // --------------------------------------------------
        // IMPORTANT:
        // productId makes this record Plan 736 specific
        // --------------------------------------------------
        ...
        if (existing) {
          ...
          if (existingRate === rate) {
            ...
            console.log(
              `♻ Updated 736 Rate: Age ${entryAge}, Term ${riderTerm} | ` +
        ...
        console.log(
          `➕ Inserted 736 Rate: Age ${entryAge}, Term ${riderTerm}, Rate ${rate}`
        );
      }
    }
    ...
    // --------------------------------------------------
    console.log("\n=================================");
    console.log("       PLAN 736 RIDER SEED");
    console.log("=================================");
    ...
    console.log(`✔ Age Range   : 18 - 59`);
    console.log(`✔ Term Range  : T16, T21, T25`);
